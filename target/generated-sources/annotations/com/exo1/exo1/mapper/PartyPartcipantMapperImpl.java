...
@Component
public class PartyPartcipantMapperImpl implements PartyPartcipantMapper {

    @Override
    public PartyParticpantDTO toDTO(PartyParticipant partyParticipant) {
        if ( partyParticipant == null ) {
            return null;
        }

        PartyParticpantDTO partyParticpantDTO = new PartyParticpantDTO();

        partyParticpantDTO.setId( partyParticipant.getId() );
        if ( partyParticipant.getAccepted() != null ) {
            partyParticpantDTO.setAccepted( partyParticipant.getAccepted() );
        }
        partyParticpantDTO.setParty( partyToPartyDTO( partyParticipant.getParty() ) );
        partyParticpantDTO.setUser( usersToUsersDTO( partyParticipant.getUser() ) );

        return partyParticpantDTO;
    }

    @Override
    public PartyParticipant toEntity(PartyParticpantDTO partyParticpantDTO) {
        if ( partyParticpantDTO == null ) {
            return null;
        }

        PartyParticipant partyParticipant = new PartyParticipant();

        partyParticipant.setId( partyParticpantDTO.getId() );
        partyParticipant.setAccepted( partyParticpantDTO.isAccepted() );
        partyParticipant.setParty( partyDTOToParty( partyParticpantDTO.getParty() ) );
        partyParticipant.setUser( usersDTOToUsers( partyParticpantDTO.getUser() ) );

        return partyParticipant;
    }

    @Override
    public List<PartyParticpantDTO> toDTOs(List<PartyParticipant> partyParticipants) {
        if ( partyParticipants == null ) {
            return null;
        }

        List<PartyParticpantDTO> list = new ArrayList<PartyParticpantDTO>( partyParticipants.size() );
        for ( PartyParticipant partyParticipant : partyParticipants ) {
            list.add( toDTO( partyParticipant ) );
        }
    ...
    @Override
    public List<PartyParticipant> toEntities(List<PartyParticpantDTO> partyParticipantDTOS) {
        if ( partyParticipantDTOS == null ) {
            return null;
        }

        List<PartyParticipant> list = new ArrayList<PartyParticipant>( partyParticipantDTOS.size() );
        for ( PartyParticpantDTO partyParticpantDTO : partyParticipantDTOS ) {
            list.add( toEntity( partyParticpantDTO ) );
        }
